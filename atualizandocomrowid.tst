...
begin
  ...
  vQtde := 0;
  for c_msg in (select i.edi_integra_col02 CodVerber,
                ...
                where upper(i.edi_integra_col01) = '220627170946786.CSV' -- '5500058294-V-NO.csv' --'5500058294-V-SP.csv' --'LOTACAOVALE.csv'
                  and trim(i.edi_integra_col02) <> 'VERBA')
   loop
     vQtde := vQtde + 1;
     Begin
         ...
         i := sql%rowcount;
         If i = 0 Then
            update tdvadm.t_slf_clienteped km
              set km.slf_clienteped_valor = vValor
             Where km.rowid = c_msg.vrowid;
           If sql%rowcount = 0 Then
              vNgravado := vNgravado + 1; 
              dbms_output.put_line('Este registro nao foi gravado ' || c_msg.vrowid || ' Valor ' || c_msg.valor);
           End If;
         End If;
         commit;
     exception
       When OTHERS Then
          verro := verro + 1;
          dbms_output.put_line('Este registro nao foi gravado ' ||  c_msg.vrowid || ' Valor ' || c_msg.valor || chr(10) || 'erro: ' || sqlerrm);
       End;
   End Loop;
